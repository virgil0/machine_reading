...
class CanonicalQueryHandler{
	...
	private Iterable<String> turnFinalResultsIntoStrings() {
		...
		for(Vertex v:this.currentVertices){
			...
			if(keys.contains(noun)){
				...
				Object fromGraphName = v.getProperty("name");
				if(fromGraphName==null) continue;
				else{
					String name;
					try {
						name = ((JSONObject)((JSONArray)fromGraphName).get(0)).getString("value");
					} catch (JSONException e) {
						e.printStackTrace();
						name="";
					}
					System.out.println("name as being passed from QueryHandler: "+name);	
					list.add(name);
				}
			}
		}
	...
	private void doMiddleTraverseChunk(CanonicalChunk chunk) {
		...
		if(noun!=null){
			System.out.println("doing a non null traversal");
			for(Vertex currentV: this.currentVertices){
			...
			System.out.println("doing a null traversal");
			...
			System.out.println("this should be 0!! "+results.size());
	...
	private boolean vertexMatchesChunk(Vertex currentV, CanonicalChunk chunk) {
		...
		if(verb.equals(Verb.TEXT)){
			...
			if(map.contains("type")&&currentV.getProperty("type").toString().equals(noun)){
				...
				System.out.println("currentV's type"+ currentV.getProperty("type"));
				System.out.println("noun: "+noun);
				
				System.out.println("24 shouldn't be here" + currentV.toString());
				return true;
			}
